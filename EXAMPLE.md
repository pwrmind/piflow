Отличный пример! Вот как можно использовать Π-топологию для задачи распределения товаров по складам:
...
### **Основная задача (2 склада):**
1. **Ограничения:**
   - Общий объем товаров: 200 тонн
   - Склад1: макс. 100 тонн
   - Склад2: макс. 150 тонн

2. **Затраты:**
   - Хранение: 5 у.е./т на Складе1, 3 у.е./т на Складе2
   - Доставка: 2 у.е./т до Склада1, 4 у.е./т до Склада2

3. **Цель:** найти распределение, минимизирующее:
   ```
   Общие затраты = (5+2)*X + (3+4)*Y
   где X - товары на Складе1, Y - товары на Складе2
   ```

### **Сложный сценарий (3 склада):**
- Учитывает третий склад с ограничением 120 тонн
- Автоматически вычисляет количество на третьем складе как остаток
- Ищет глобальный минимум затрат

### **Что получаем на выходе:**
1. **Оптимальное распределение** товаров
2. **Детализацию затрат** по каждому складу
3. **Топ-5 лучших вариантов** для анализа альтернатив
4. **Визуализации:**
   - Допустимая область распределения
   - Зависимость затрат от распределения
   - Структура затрат оптимального решения

## Преимущества Π-топологии для этой задачи:

1. **Полный перебор** - находим ВСЕ допустимые распределения
2. **Учет всех ограничений** одновременно
3. **Быстрое вычисление** на GPU (десятки тысяч вариантов за секунды)
4. **Гибкость** - легко добавить новые ограничения
5. **Визуализация** пространства решений

## Практическое применение:

- **Логистика** - распределение товаров между региональными складами
- **Снабжение** - оптимизация поставок в розничные точки
- **Складское хозяйство** - планирование загрузки складов
- **Цепочки поставок** - минимизация логистических издержек

Программа позволяет быстро проанализировать сотни тысяч вариантов распределения и выбрать оптимальный с учетом всех ограничений и стоимостных факторов.